chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if (request.message == "Item created") window.location.reload();
  else {
...
chrome.tabs.query({ active: true, lastFocusedWindow: true }, function (tabs) {
  ...
  var apiSku =
    ...
    url.replace("https://marketplace.tf/items/tf2/", "");
  var label = document.getElementById("spanInfo");
  ...
  if (url.includes("https://marketplace.tf/items/tf2/")) {
    fetch(apiSku, {
      ...
      .then((data) => {
        if (data.message == "Item not found") {
        ...
        } else if (data.message == "Item found") {
          document.body.style.height = "60px";
          ...
          document.getElementsByClassName("btn")[1].onclick = function () {
            ...
            if (updateItem.name == "") {
              ...
              document.getElementById("isCraftable").value =
                data.data[0].isCraftable;
          ...
          document.getElementsByClassName("btn")[2].onclick = function () {
            fetch(apiSku, {
              ...
              .then((response) => response.json())
              .then((data) => {
                if (data.message == "Item deleted") window.location.reload();
                else labelSet(label, data.message);
              });
          };
        } else {
          buttonsInnerHTML("Something went wrong");
        }
      })
      .catch(() => {
        buttonsInnerHTML("Something went wrong");
      });
  } else buttonsInnerHTML("Select an Item on Marketplace.tf");
});
